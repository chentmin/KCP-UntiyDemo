...
namespace Network
{
    public class KCPSocket
    {
        ...
        public KCPSocket(int bindPort, uint kcpKey, string _netType, AddressFamily family = AddressFamily.InterNetwork)
        {
            m_AddrFamily = family;
            m_KcpKey = kcpKey;
        ...
        public string LocalIP
        {
            ...
            get { return IPManager.GetIP(ADDRESSFAM.IPv4); }
        }
        ...
        public IPEndPoint LocalEndPoint
        {
            get
            {
                if (m_LocalEndPoint == null ||
                    m_LocalEndPoint.Address.ToString() != IPManager.GetIP(ADDRESSFAM.IPv4))
                {
        ...
        private KCPProxy GetKcp(IPEndPoint ipep)
        {
            ...
            proxy = new KCPProxy(m_KcpKey, ipep, m_SystemSocket, netType);
    ...
    class KCPProxy
    {
        ...
        public void Dispose()
        {
            ...
            if (m_Kcp != null)
            {
                //m_Kcp.Dispose();
                //m_Kcp = null;
                KCP.Setoutput(m_Kcp, null);
                m_Kcp = System.IntPtr.Zero;
            }
        ...
        public bool DoSend(byte[] buff, int size)
        {
            ...
            return KCP.Send(m_Kcp, buff, size) >= 0;
        ...
        private void HandleRecvQueue()
        {
            ...
            while (!m_RecvQueue.Empty())
            {
                ...
                int ret = KCP.Input(m_Kcp, recvBufferRaw,0,1024);
                ...
                m_NeedKcpUpdateFlag = true;
                var peekSize = KCP.Peeksize(m_Kcp);
                //for (int size = m_Kcp.PeekSize(); size > 0; size = m_Kcp.PeekSize())
                for (int size = peekSize; size > 0; size = peekSize)
                {
                    var recvBuffer = new byte[size];
                    if (KCP.Recv(m_Kcp,recvBuffer,size) > 0)
        ...
        public void Update(uint currentTimeMS)
        {
            ...
            if (m_NeedKcpUpdateFlag || currentTimeMS >= m_NextKcpUpdateTime)
            {
                ...
                KCP.Update(m_Kcp, currentTimeMS);
                m_NextKcpUpdateTime = KCP.Check(m_Kcp, currentTimeMS);
